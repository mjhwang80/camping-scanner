...
class CampingtalkQMonitor(CampingMonitor):  
    ...
    def __init__(self):
        ...
        self.client = httpx.AsyncClient(verify=False,follow_redirects=True,cookies={"auth": "token"}, timeout=10.0)
    ...
    async def check_availability(self, params: dict):
        ...
        try:
            response = await self.client.get(url, headers=current_headers,  cookies={"auth": "token"}, timeout=10.0)
            ...
            if found_sites:
                ...
                #logger.info(f"{campsite_name} 예약 URL: {link}")
    ...
    async def close_client(self):
        if self.client and not self.client.is_closed:
            ...
            logger.info("[*] [캠핑톡] httpx AsyncClient 커넥션 풀을 안전하게 닫았습니다.")          