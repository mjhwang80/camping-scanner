...
class InterparkMonitor(CampingMonitor):  
    ...
    async def close_client(self):
        print(f"[*] [인터파크] close_client 호출됨. self.client 상태: {self.client}")
        logger.info(f"[*] [인터파크] close_client 호출됨. self.client 상태: {self.client}")
        if self.client and not self.client.is_closed:
            await self.client.aclose()
            logger.info("[*] [인터파크] httpx AsyncClient 커넥션 풀을 안전하게 닫았습니다.")
