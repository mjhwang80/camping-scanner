...
const Comp = {
  ...
  bindEvents: function () {
    ...
    document
      ...
      .addEventListener("click", () => this.stopWatching()); //감시중지
    document
      .getElementById("homePageBtn")
      .addEventListener("click", () => this.openHomePage()); //홈페이지 열기
    document
      .getElementById("watchDate")
      .addEventListener("change", () => this.onChangeWatchDate()); //날짜 변경 체크(인터파크)
  },
  ...
  changeFlatform: async function () {
    ...
    try {
      ...
      listContainer.innerHTML =
        '<li class="p-4 text-center text-red-500">데이터를 불러오지 못했습니다.</li>';
    }
  },
  ...
  updateDetailPanel: function (site) {
    ...
    const name =
      site.getElementsByTagName("name")[0]?.textContent || "이름 없음";
    const sites = Array.from(site.getElementsByTagName("site"))
      .map((s) => s.textContent)
      .join(", ");
    const type =
      site.getElementsByTagName("type")[0]?.textContent || "정보 없음";
  ...
  generatorMaxDayOption: function (site) {
    ...
    if (type === "Interpark") {
      ...
      const maxStayDay =
        site.getElementsByTagName("maxStayDay")[0]?.textContent || 2; //최대 예약 박수
...
const Alert = {
  ...
  connect: function () {
    ...
    socket.onclose = () => {
      ...
      console.warn(
        `연결 끊김. ${delay / 1000}초 후 재연결 시도... (횟수: ${this.retryCount})`,
      );
