...
- /camping-scanner/app/platforms/gtdc.py # gtdc 크롤링 로직
- /camping-scanner/app/platforms/foresttrip.py # foresttrip 크롤링 로직
...
- index.html에 "검색후 감시 종료", "검색후 브라우저 열기", "자동 예약 요청" 버튼을 상단 "감시 시작" 버튼 앞으로 위치하게 하고 싶어.
- index.html에 실행 방식, 최대 요청 주기를 사이트, 연박 필드 사이에 넣고 싶어.
        ...
        <header class="w-full h-14 bg-white border-b shadow-sm flex items-center justify-between px-4 md:px-6 z-20">
            <div class="flex items-center space-x-2">
                ...
                <h1 class="text-xl font-black text-slate-800">
                    캠핑가자
                    <span class="text-slate-400 text-xs font-medium">ver 1.4.2</span>
                </h1>
            </div>
            ...
            <main class="flex-1 flex flex-col overflow-hidden bg-slate-50">
                <div class="flex-1 p-2 md:p-4 grid grid-cols-12 gap-2 md:gap-4 overflow-y-auto custom-scrollbar">
                    <section id="reservationPanel" class="col-span-12 lg:col-span-5 bg-white rounded border border-slate-300 flex flex-col shadow-sm">
                        <div class="p-3 border-b bg-slate-50 flex justify-between items-center flex-wrap gap-2">
                            <h2 class="font-black text-slate-700 text-sm"><i class="fa-solid fa-pen-to-square mr-1"></i> 예약 정보</h2>

                            <div class="flex items-center space-x-3 ml-auto flex-wrap gap-y-1">
                                <label class="flex items-center font-bold text-slate-600 text-[11px] cursor-pointer">
                                    <input type="checkbox" checked id="findNextRun" class="mr-1 w-3.5 h-3.5 accent-indigo-600" />
                                    검색후 감시 종료
                                </label>

                                <label class="flex items-center font-bold text-slate-600 text-[11px] cursor-pointer">
                                    <input type="checkbox" checked id="findNextOpenBroswer" class="mr-1 w-3.5 h-3.5 accent-indigo-600" />
                                    검색후 브라우저 열기
                                </label>

                                <div id="autoReserveField" class="hidden">
                                    <label class="flex items-center font-bold text-red-600 text-[11px] cursor-pointer">
                                        <input type="checkbox" id="autoReserve" name="autoReserve" class="mr-1 w-3.5 h-3.5" />
                                        자동 예약 요청
                                    </label>
                                </div>
                        ...
                        <div class="p-4 space-y-4 text-sm">
                            ...
                            <div class="flex items-center pt-1">
                                <label class="w-24 font-bold text-slate-600 text-xs">실행 방식 :</label>
                                <select class="flex-1 p-1.5 border rounded text-xs bg-white" id="execTypeSelect">
                                    <option value="NOW" selected>즉시 실행</option>
                                    <option value="RESERVED">예약 실행</option>
                                </select>
                            </div>

                            <div id="reservedTimeFields" class="hidden flex items-center">
                                <label class="w-24 font-bold text-slate-600 text-xs">예약 시간 :</label>
                                <input type="datetime-local" id="reservedDateTime" step="1" class="flex-1 p-1.5 border rounded text-xs" />
                            </div>

                            <div class="flex items-center">
                                <label class="w-24 font-bold text-slate-600 text-xs">최대요청주기 :</label>
                                <select class="flex-1 p-1.5 border rounded text-xs bg-white" id="requestInterval">
                            ...
                            <div id="loginFields" class="hidden space-y-3 pt-2 border-t">
                                <div class="flex items-center">
...
async def start_monitor(params: dict = Body(...), background_tasks: BackgroundTasks = None):
    ...
    elif platform_type == "Gtdc":
            monitor = GtdcMonitor()
    elif platform_type == "Foresttrip":
